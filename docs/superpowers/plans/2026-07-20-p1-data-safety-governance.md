...
- [ ] 先写 catalog 测试，断言 `governance_settings`、`backup_records`、`restore_preflights`、`audit_logs` 的列、枚举、索引与 AuditLog UPDATE/DELETE 拒绝 trigger。
- [ ] 运行 `pnpm --dir backend test:integration -- --runInBand data-governance-catalog.spec.ts`，确认 RED。
- [ ] 在 Prisma schema 与前向 migration 中实现规格模型；自动备份日期建立幂等唯一约束，审计表 trigger 只允许 INSERT。
- [ ] 运行 `pnpm --dir backend prisma:generate`、目标集成测试与 `pnpm --dir backend build`。
...
- [ ] 先覆盖 POSIX/Windows 路径逃逸、symlink、重复 manifest path、流式 hash/copy、原子 rename、超时和 `shell:false` 参数白名单的失败测试。
- [ ] 运行目标测试，确认现有适配器无法通过。
- [ ] 扩展 StoragePort 的 stream/stat/walk/statfs/rename 能力；以 `path.relative` 和 `lstat/realpath` 双重 containment 取代字符串前缀判断。
- [ ] 实现只接受服务端构造 argv 的 ProcessRunner，截断并净化 stderr，不把 URL/secret 写入日志。
- [ ] 运行目标测试、storage integration test、lint 和 build。
...
- [ ] 先写 dump 失败、文件复制失败、manifest 篡改、并发锁、成功状态转换与敏感错误脱敏测试。
- [ ] 运行目标测试，确认 RED。
- [ ] 实现 `POST/GET/verify/DELETE /api/governance/backups`，固定使用 custom dump、临时目录、fsync/rename 和 SHA-256 manifest。
- [ ] 使用 advisory lock + 进程 mutex；只有 dump、文件和 manifest 二次校验都成功才标记 CREATED/VERIFIED。
- [ ] 运行单元/集成测试、build、`git diff --check`。
...
- [ ] 先用 fake timers 覆盖本地时区、重启幂等、同日三次上限、成功日期推进、保护快照不清理。
- [ ] 运行测试，确认 RED。
- [ ] 仿照 ReminderScheduler 实现 `setInterval().unref()`、advisory lock 与关闭清理；保留策略只删除允许删除的成功备份。
- [ ] 运行目标测试及 `pnpm --dir backend test:unit -- --runInBand`。
...
- [ ] 先覆盖 token 过期/复用、TOCTOU manifest 变化、空间不足、工具版本不兼容、dump/文件交换失败和回滚再失败测试。
- [ ] 运行目标测试，确认 RED。
- [ ] 实现只读 preflight、10 分钟单次 confirmation token、PRE_RESTORE 快照、staging、journal 与固定 `pg_restore --single-transaction --exit-on-error --clean --if-exists --no-owner --no-privileges`。
- [ ] 维护 CLI 在 Prisma 断开后运行；成功后执行 migration fingerprint、FK、核心计数和文件 hash 验证；失败按 journal 恢复 DB 和目录。
...
- [ ] 先写字段值/正文/手机号/URL query/凭据永不入审计、写失败不伪装成功、健康检查识别 schema drift/缺失文件/最近备份的测试。
- [ ] 运行测试，确认 RED。
- [ ] 实现全局写请求字段名审计和领域精确审计；业务变更与审计必须接收同一个 Prisma transaction。
- [ ] 实现快速 health 与显式深度文件扫描，检查只读，不自动修复。
- [ ] 运行目标测试、integration suite、lint 和 build。
...
- [ ] 先写 IPC allowlist、renderer 看不到 maintenance token、停止后端→运行 CLI→重启、失败重启与错误净化测试。
- [ ] 运行 `pnpm --dir desktop test`，确认 RED。
- [ ] 主进程生成随机维护令牌，只暴露 `chooseBackupDirectory()` 与 `restoreBackup(id, expectedHash)`；禁止通用 shell/path/fetch IPC。
- [ ] 执行 desktop test/typecheck/build dry-run。
...
- [ ] 先写设置入口、备份步骤、强确认、状态轮询、浏览器/Electron 分支、审计过滤和健康空/错态测试。
- [ ] 运行目标 Vitest，确认 RED。
- [ ] 用 Semi Design 实现 `/settings/data-governance` 四页签，不增加一级导航。
- [ ] 运行 frontend typecheck、目标测试、build 与 Playwright 备份/预检 smoke。
