...
- [ ] 只查阅 OpenAI、阿里云短信、CalDAV/WebDAV 标准或服务商官方文档，记录请求/响应、认证、限流、错误分类、隐私和 SDK/version。
- [ ] 对 OpenAI 使用 `openai-docs` skill；技术检索只引用第一方来源。
- [ ] 固定 provider contract 与超时/大小/重试策略；没有官方依据的字段不得进入生产 adapter。
...
- [ ] 先写 Profile/Run/Recipient/Delivery/ObjectLink catalog 和 PENDING→RUNNING→终态合法转换测试。
- [ ] 运行测试确认 RED。
- [ ] 实现 schema、迁移、索引、手机号 mask 约束和纯状态机；ExtensionRun 不保存正文。
...
- [ ] 先写 safeStorage 不可用、0600/原子写、损坏 vault、renderer 无读取 secret/任意 IPC/fetch、provider/operation/payload allowlist 测试。
- [ ] 运行 desktop tests 确认 RED。
- [ ] 实现只允许 put/has/delete 的 vault 和 typed execute bridge；生产日志不得包含 secretObject。
- [ ] 运行 desktop test/typecheck 和 preload API 快照测试。
...
- [ ] 先写 publicConfig 递归 secret key 拒绝、禁用/无凭据拒绝、一次性 run token、payload hash 与重复 complete 幂等测试。
- [ ] 运行测试确认 RED。
- [ ] 实现 Profile CRUD/连接测试、Run prepare/complete 和 LOCAL_PREVIEW/LOCAL_MANUAL；预览必须标记 PREVIEW/REJECTED，不能伪造发送成功。
- [ ] 运行 backend/desktop 目标门禁。
...
- [ ] 先写 important+SMS+recipient+template 四条件、页面通知不被阻塞、离线重试、4xx 不重试、429/5xx 指数退避、日志 mask 测试。
- [ ] 运行目标测试确认 RED。
- [ ] 实现 delivery 状态机、`/extensions` socket request、Electron provider 和一次性完成回调；providerMessageId/成本元数据白名单入审计。
- [ ] 运行目标测试、backend build、desktop typecheck。
...
- [ ] 先写 40k/50k 字符限制、最多 8 片段、citation allowlist、正文不入 Run、未确认不执行、未采纳不写业务对象测试。
- [ ] 运行测试确认 RED。
- [ ] 实现 prepare 返回数据离开本机摘要与 confirmation hash；执行 OpenAI Responses typed schema，拒绝未知 citation；采纳复用现有会议/文档/任务 API。
- [ ] 运行 backend/desktop 目标测试和敏感信息扫描。
...
- [ ] 先写默认 pull-only、派生日程只读、remote root 路径逃逸、跨 host redirect、hash/version 冲突与三种冲突选择测试。
- [ ] 运行目标测试确认 RED。
- [ ] 实现 preflight 只返回 add/update/conflict；commit 校验 preflight hash 后写入，失败不修改本地对象。
- [ ] 运行 backend/desktop 目标门禁。
...
- [ ] 先写四页签、凭据不可用、连接测试、数据离开本机确认、运行日志 mask、AI 采纳与同步冲突测试。
- [ ] 运行目标 Vitest 确认 RED。
- [ ] 用 Semi Design 实现 `/settings/extensions`；没有 Electron/凭据时清楚降级且不影响本地功能。
- [ ] 运行 frontend typecheck/test/build 和浏览器 smoke。
