...
| 21 | 知识库文件管理 | 文件浏览器 + 增删改查 + 批量操作 + 拖拽上传 + 文件→文档转换 |
...
后端 `FilesService` 已具备完整的 FileAsset CRUD（创建、列取、更新、移到回收站、恢复、添加版本），但前端 `KnowledgeHomePage` 缺少文件管理界面。文件只能作为文档附件查看，不能独立浏览、上传和管理。用户无法：
- 浏览知识库里的所有文件
- 上传文件并独立管理（不绑定到某个文档）
- 重命名、移动、下载文件
- 批量操作（批量删除、批量移动）
...
│          │                       │                        │
│ ▼ 全部   │ ┌───────────────────┐ │  方案设计_v3.docx      │
│   • 文档 │ │ 📄 方案设计.docx  │ │  类型：DOCX            │
│   • 文件 │ │ 📄 复盘记录.md   │ │  大小：156 KB           │
│          │ │ 📊 数据报表.xlsx │ │  版本：3（最新）        │
│ ▼ 空间   │ │ 📄 接口文档.md   │ │  上传时间：07-26       │
│   • 产品 │ │                   │ │                        │
│   • 技术 │ │ [上传文件]        │ │  [下载] [删除] [重命名] │
│   • 复盘 │ │                   │ │  [上传新版本] [转文档]  │
│          │ │                   │ │                        │
...
- 左侧目录树：**全部文档** / **全部文件** / **空间列表**（文档和文件共享空间体系）
- 中间文件列表：表格展示（文件名、类型图标、大小、版本数、修改时间）+ 搜索 + 排序
- 右侧详情面板：文件元信息 + 操作按钮 + 版本历史

### 21.3 文件 CRUD 操作

| 操作 | API（已有或新增） | 说明 |
|------|-----------------|------|
| 上传文件 | POST `/api/files` (已有) | 支持拖拽/点击上传，MIME 校验 |
| 列取文件 | GET `/api/files` (已有) | 按空间/标签/搜索过滤，分页排序 |
| 重命名 | PATCH `/api/files/:id` (已有) | 改 displayName |
| 移动到空间 | PATCH `/api/files/:id` | 改 spaceId，可以移出空间（spaceId=null） |
| 下载 | GET `/api/files/:id/download` (新增) | 返回原始文件流 + Content-Disposition |
| 移到回收站 | DELETE `/api/files/:id` (已有) | 软删除，30 天后自动清理 |
| 恢复 | POST `/api/files/:id/restore` (已有) | 从回收站恢复 |
| 永久删除 | DELETE `/api/files/:id/permanent` (新增) | 二次确认，删除存储文件 |
| 上传新版本 | POST `/api/files/:id/versions` (已有) | 保留版本历史，SHA-256 去重 |
| 批量删除 | POST `/api/files/batch` (新增) | `{ ids: [...], action: 'trash' }` |
| 批量移动 | POST `/api/files/batch` (新增) | `{ ids: [...], action: 'move', spaceId: '...' }` |

### 21.4 文件上传增强

在现有单文件上传基础上，增加：

- **拖拽上传**：从操作系统拖文件到知识库页面 → 自动上传到当前所在空间
- **批量上传**：一次选多个文件（上限 10 个），逐个上传并显示进度
- **格式支持**：TXT、MD、DOCX、PDF、XLSX、PPTX、图片（PNG/JPG/GIF/SVG）、代码文件（TS/JS/PY/JSON/YAML 等）
- **大小限制**：单文件 50 MiB，单次批量总共 200 MiB
- **重名处理**：同名文件自动加序号后缀（方案设计.docx → 方案设计 (1).docx），不覆盖
- **SHA-256 去重**：内容完全相同的文件跳过上传，提示"该文件已存在：[文件链接]"

### 21.5 文件 → 文档转换

非纯文本文件（DOCX/PDF）上传后，用户可以手动转换为知识库文档以纳入 RAG 索引：

```
文件详情右侧面板 → [转为文档] 按钮
  → 后端提取文本 → 创建 ContentDocument(title=文件名, plainText=提取文本)
  → 原文件关联为文档附件（fileAssetId）
  → 自动触发分块 + embedding → 可被 AI 问答检索
```

纯文本文件（TXT/MD）上传时自动转换（因为 100% 无损），DOCX/PDF 手动触发（因为可能丢失格式）。

### 21.6 回收站统一

文档和文件共用回收站视图，前端增加一个 Tab `回收站`，列出所有已删除的文档和文件：

- 显示：名称、类型图标（文档/文件）、删除时间、剩余天数
- 操作：恢复、永久删除
- 批量：全选 → 批量恢复/批量永久删除
- 自动清理：超过 30 天的项目后端定时任务清理（已有 `expiresAt`）

### 21.7 现有组件复用

- `FileAttachments` 组件已有文件上传/列表/下载逻辑，扩展为独立文件浏览器
- `FilesService` 后端 CRUD 完备，只缺批量操作和下载端点
- 文件存储层（`StoragePort`）已稳定，上传/下载/删除路径不变
