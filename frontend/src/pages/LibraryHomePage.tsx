...
function RecordDetailValue({
  ...
  if (error) {
    const text =
      error.code === 'DIV_ZERO' ? '#DIV/0!' : error.code === 'CYCLE' ? '#CYCLE!' : '⚠ 计算错误'
    return (
      <strong className="base-record-detail__error" title={error.message}>
        {text}
  ...
  return (
    <strong>
      ...
      {['LOOKUP', 'ROLLUP', 'FORMULA'].includes(field.type) ? (
        <small>
          <ComputedFieldExplanation field={field} fields={fields} tables={tables} />
        </small>
      ) : null}
    </strong>
  )
...
export default function LibraryHomePage() {
  ...
  const selectedView =
    ...
    views[0] ??
    null
  const resolvedView = useMemo<DataView | null>(
    () =>
      selectedView
        ? {
            ...selectedView,
  ...
  const viewConfigSave = useDebouncedViewConfigSave(async (id, config) => {
    ...
    try {
      ...
      if (latestDraftConfigs.current.get(id) === config) {
        ...
        if (serverConfig) latestDraftConfigs.current.set(id, serverConfig)
        else latestDraftConfigs.current.delete(id)
        Toast.error('视图配置保存失败。已恢复服务端配置。')
      }
    } finally {
      setConfigSaveCount((count) => Math.max(0, count - 1))
    }
  }, VIEW_SAVE_DELAY_MS)
  ...
  async function openCreateRecordForm() {
    ...
    const created = await runViewOperation(async () => {
      const next = await createBaseView(selectedTable.id, {
        name: '表单',
        type: 'FORM',
        config: {},
      })
  ...
  if (workspacesQuery.isPending) {
    return (
      <div className="base-loading">
        <Skeleton loading placeholder={<Skeleton.Paragraph rows={8} />} />
        <p>正在加载多维表格…</p>
      </div>
    )
  }
    ...
    return (
      <div className="base-loading">
        <Banner
          ...
          fullMode={false}
          title="无法读取多维表格"
          description="请确认本地后端和 PostgreSQL 已启动。"
          closeIcon={null}
        >
  ...
  if (!workspace)
    return (
      <div className="base-loading">
        <Empty title="还没有数据工作区" description="后端将在首次访问时创建研发工作台。" />
      </div>
    )
    ...
    <div className="base-page">
          ...
          <>
            ...
            <ViewManager
              ...
              onDelete={(id) => {
                ...
                return runViewOperation(async () => {
                  await deleteBaseView(id)
                  await refreshViews()
                }, '删除视图失败。')
              }}
              onSetDefault={(id) =>
                runViewOperation(async () => {
                  await updateBaseView(id, { isDefault: true })
                  await refreshViews()
                }, '设置默认视图失败。')
              }
            />
            <div className="base-page__content">
              {recordsQuery.isPending ? (
                <Skeleton loading placeholder={<Skeleton.Paragraph rows={10} />} />
              ) : null}
              {recordsQuery.isError ? (
                <Banner
                  ...
                  fullMode={false}
                  title="无法读取数据记录"
                  description="数据表结构已加载，但记录服务暂时不可用。"
                  closeIcon={null}
                >
              ...
              {recordsQuery.data && resolvedView ? (
                resolvedView.type === 'GRID' ? (
                  <GridView
                    ...
                    onViewChange={saveViewConfig}
                  />
                ) : resolvedView.type === 'KANBAN' ? (
                  <KanbanView
                    fields={fields}
                    records={records}
                    groupFieldKey={String(resolvedView.config.groupField ?? '') || undefined}
                    isUpdating={updateRecordMutation.isPending}
                    onGroupFieldChange={(groupField) =>
                      saveViewConfig({ ...resolvedView.config, groupField })
                    }
                    onRecordUpdate={(recordId, input) =>
                  ...
                  <CalendarView
                    fields={fields}
                    records={records}
                    dateFieldKey={String(resolvedView.config.dateField ?? '') || undefined}
                  ...
                  <FormView
                    tableSource={selectedTable.source}
                    fields={fields}
                    tables={tables}
      ...
      {selectedTable ? (
        <FieldManager
          ...
          onCreateField={(input) =>
            createFieldMutation.mutateAsync({ tableId: selectedTable.id, input })
          }
      ...
      >
        {selectedRecord && selectedTable ? (
          <div className="base-record-detail">
            {fields.map((field) => (
              <div key={field.id}>
            ...
            {selectedRecord.sourcePath ? (
              <Link to={selectedRecord.sourcePath}>打开原业务对象 →</Link>
            ) : null}
          </div>
        ) : null}
      </SideSheet>
