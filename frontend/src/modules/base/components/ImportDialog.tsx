  ...
  return (
    <Modal title={`导入到 ${table.name}`} visible={visible} footer={null} onCancel={onClose} width={760}>
      <div className="base-import-dialog">
        ...
        {step === 1 && source ? <section><h3>选择工作表</h3><select aria-label="工作表" value={sheet} onChange={(event) => setSheet(event.target.value)}>{source.sheetNames.map((name) => <option key={name}>{name}</option>)}</select><Button theme="solid" type="primary" loading={pending} onClick={() => void inspectSheet()}>继续字段映射</Button></section> : null}
        {step === 2 && source ? <section className="base-import-dialog__mapping"><header><h3>字段映射</h3><p>类型建议只用于参考；未映射列会被忽略。</p></header>{source.columns.map((column, index) => <label key={column}><span>{column}<small>建议 {source.inferredTypes[column]}</small></span><select aria-label={`${column} 映射`} value={mappingValue(index)} onChange={(event) => updateMapping(index, event.target.value)}><option value="ignore">忽略此列</option>{writableFields.map((field) => <option key={field.id} value={`field:${field.id}`}>{field.name} · {field.type}</option>)}<option value="new:TEXT">新建文本字段</option><option value="new:LONG_TEXT">新建多行文本字段</option><option value="new:NUMBER">新建数字字段</option><option value="new:DATETIME">新建日期字段</option><option value="new:SINGLE_SELECT">新建单选字段</option><option value="new:MULTI_SELECT">新建多选字段</option><option value="new:CHECKBOX">新建勾选字段</option><option value="new:LINK">新建链接字段</option></select></label>)}<Button theme="solid" type="primary" loading={pending} onClick={() => void preview()}>全量预检</Button></section> : null}
