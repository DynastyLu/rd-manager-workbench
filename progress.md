...
- **状态：** complete
- 已确认用户截图不是源文件损坏，而是统一 PDF 预览架构造成的格式错配。
- 已读取问题 XLS 的真实元数据与内容：2 个工作表，主表有效内容 5 行 × 8 列，编码表 3730 行 × 2 列。
- 已确认字体替换错误：“等线/宋体”被匹配到不含中文字形的 Verdana/Times New Roman。
- 已新增文件类型路由、Excel/XLS 工作簿数据接口与工作簿阅读器；真实问题文件在页面显示 2 个工作表、完整中文单元格，编码表按 100 行分页，共 38 页。
- 已新增 DOCX 原包排版阅读器，真实项目立项计划书按 Word 页面、表格和中文正文正常显示，不再先转换成 PDF。
- PDF、图片、文本、Markdown、JSON、HTML 已分流到各自阅读方式；旧 DOC/PPT 等保留 LibreOffice PDF 兜底。
- LibreOffice 兜底改用隔离 profile、中文字体别名和 v2 缓存；真实 DOCX 兼容转换生成 9 页 PDF，`pdftotext` 中文完整，嵌入字体为 STSongti SC。
- 验证：后端工作簿/Office 合约测试、后端 unit、前端知识库测试、前端 typecheck/Lint/build、后端 build 全部通过；Chromium 真实页面无控制台错误。
