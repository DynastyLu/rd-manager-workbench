...
describe('Multi-dimensional base API', () => {
  ...
  beforeAll(async () => {
    ...
    authenticated = await authenticatedRequest(app, prisma, 'EMPLOYEE', [
      { code: 'base.read', dataScope: 'INVOLVED' },
      { code: 'base.create', dataScope: 'INVOLVED' },
      { code: 'base.update', dataScope: 'INVOLVED' },
      { code: 'base.delete', dataScope: 'INVOLVED' },
    ]);
  });
  ...
  it('creates the default workspace and five presets idempotently under concurrency', async () => {
    const responses = await Promise.all(
      Array.from({ length: 2 }, () =>
        authenticated.agent.get('/api/base/workspaces').expect(200),
      ),
    );
  ...
  it('supports custom table, field, record and view CRUD with field validation', async () => {
    ...
    await authenticated.agent
      ...
      .expect(400);
    const record = await authenticated.agent
      .post(`/api/base/tables/${tableId}/records`)
      .send({
        values: {
          title: '任务 A',
    ...
    await authenticated.agent
      ...
      .expect(200);
    const view = await authenticated.agent
    ...
    await authenticated.agent
      ...
      .expect(204);
    await authenticated.agent.delete(`/api/base/views/${view.body.data.id}`).expect(204);
  });
  ...
  it('synchronizes a two-way relation when creating and updating custom records', async () => {
    ...
    const relation = await authenticated.agent
      .post(`/api/base/tables/${sourceTable.body.data.id}/fields`)
      .send({
        key: 'targets',
        name: '目标',
    ...
    const target = await authenticated.agent
  ...
  it('enforces relation cardinality and serializes concurrent one-to-one claims', async () => {
    ...
    const oneToOne = await authenticated.agent
    ...
    const competingTarget = await authenticated.agent
      .post(`/api/base/tables/${targetTableId}/records`)
  ...
  it('synchronizes one-to-many and many-to-many swaps and diffs without duplicates', async () => {
    ...
    const sourceTableId = sourceTable.body.data.id as string;
    const targetTableId = targetTable.body.data.id as string;
    const manyToMany = await authenticated.agent
      .post(`/api/base/tables/${sourceTableId}/fields`)
      .send({
        key: 'targets',
        name: '多个目标',
    ...
    const oneToMany = await authenticated.agent
      .post(`/api/base/tables/${sourceTableId}/fields`)
      .send({
        key: 'ownedTargets',
        name: '独占多个目标',
  ...
  it('validates relation value shape, uniqueness and custom target ownership', async () => {
    ...
    const target = await authenticated.agent
      ...
      .expect(201);
    const wrong = await authenticated.agent
    ...
    const retained = await authenticated.agent
      .post(`/api/base/tables/${sourceTable.body.data.id}/records`)
  ...
  it('cleans inverse values on record deletion and safely decouples relation fields', async () => {
    ...
    const target = await authenticated.agent
      ...
      .send({ values: { title: '目标记录' } })
      .expect(201);
    const deletedSource = await authenticated.agent
      .post(`/api/base/tables/${sourceTableId}/records`)
    ...
    const retainedSource = await authenticated.agent
      .post(`/api/base/tables/${sourceTableId}/records`)
  ...
  it('rejects relation target or cardinality changes while non-empty values exist', async () => {
    ...
    const target = await authenticated.agent
      ...
      .expect(201);
    const oneWay = await authenticated.agent
      .post(`/api/base/tables/${sourceTableId}/fields`)
      .send({
        key: 'oneWayTarget',
        name: '单向目标',
      ...
      .expect(201);
    const many = await authenticated.agent
      .post(`/api/base/tables/${sourceTableId}/fields`)
      .send({
        key: 'manyTargets',
        name: '多个目标',
    ...
    const paired = await authenticated.agent
      .post(`/api/base/tables/${sourceTableId}/fields`)
      .send({
        key: 'sharedTarget',
        name: '共享目标',
    ...
    const secondSource = await authenticated.agent
      .post(`/api/base/tables/${sourceTableId}/records`)
    ...
    const emptyMany = await authenticated.agent
      .post(`/api/base/tables/${sourceTableId}/fields`)
      .send({
        key: 'emptyTargets',
        name: '空目标',
  ...
  it('rechecks archived custom tables after record mutations acquire their advisory locks', async () => {
    ...
    const updateRecord = await authenticated.agent
      .post(`/api/base/tables/${tables[1].body.data.id}/records`)
      .send({ values: { title: '更新前' } })
      .expect(201);
    const deleteRecord = await authenticated.agent
  ...
  it('validates computed field configs and previews formulas without persisting preview data', async () => {
    const workspace = await authenticated.agent
      ...
      .expect(201);
    const sourceTable = await authenticated.agent
      .post(`/api/base/workspaces/${workspace.body.data.id}/tables`)
      .send({ name: `${prefix} 计算源表` })
      .expect(201);
    const targetTable = await authenticated.agent
      .post(`/api/base/workspaces/${workspace.body.data.id}/tables`)
      .send({ name: `${prefix} 关系目标表` })
      .expect(201);
    const sourceTableId = sourceTable.body.data.id as string;
    const targetTableId = targetTable.body.data.id as string;
    const amount = await authenticated.agent
      .post(`/api/base/tables/${sourceTableId}/fields`)
    ...
    const lookup = await authenticated.agent
      .post(`/api/base/tables/${sourceTableId}/fields`)
      .send({
        key: 'projectTitle',
        name: '项目名称',
    ...
    const createdAtField = await authenticated.agent
      .post(`/api/base/tables/${sourceTableId}/fields`)
    ...
    const formula = await authenticated.agent
      .post(`/api/base/tables/${sourceTableId}/fields`)
      .send({
        key: 'total',
        name: '合计',
    ...
    const record = await authenticated.agent
      ...
      .expect(201);
    const foreignRecord = await authenticated.agent
      .post(`/api/base/tables/${targetTableId}/records`)
      .send({ values: { title: '目标记录' } })
      .expect(201);
    await authenticated.agent
  ...
  it('updates a preset task through TasksService so completion clears reminders', async () => {
    const task = await prisma.workTask.create({
      data: {
        title: `${prefix} 联动任务`,
        dueAt: new Date('2026-07-25T01:00:00.000Z'),
        createdByUserId: authenticated.user.id,
        updatedByUserId: authenticated.user.id,
        ownerUserId: authenticated.user.id,
      },
  ...
  it('resolves computed values on every custom record response and rejects computed writes', async () => {
    ...
    const score = await authenticated.agent
    ...
    await authenticated.agent
      ...
      .expect(201);
    const average = await authenticated.agent
      .post(`/api/base/tables/${sourceTableId}/fields`)
      .send({
        key: 'averageScore',
        name: '平均评分',
    ...
    const createdAtField = await authenticated.agent
      ...
      .send({ key: 'createdAt', name: '创建时间', type: DataFieldType.CREATED_AT })
      .expect(201);
    const updatedAtField = await authenticated.agent
      .post(`/api/base/tables/${sourceTableId}/fields`)
      .send({ key: 'updatedAt', name: '更新时间', type: DataFieldType.UPDATED_AT })
      .expect(201);
    await authenticated.agent
  ...
  it('executes persisted views before pagination for custom and system records', async () => {
    ...
    await authenticated.agent
      ...
      .expect(400);
    const disguisedGantt = await authenticated.agent
      .post(`/api/base/tables/${tableId}/views`)
      .send({
        name: '待转换视图',
        type: 'GRID',
    ...
    const legacyField = await authenticated.agent
      .post(`/api/base/tables/${tableId}/fields`)
      .send({ key: 'legacy', name: '已归档字段', type: DataFieldType.TEXT })
      .expect(201);
    const archivedView = await authenticated.agent
      .post(`/api/base/tables/${tableId}/views`)
      .send({
        name: '归档字段兼容',
        type: 'GRID',
    ...
    const systemView = await authenticated.agent
      .post(`/api/base/tables/${projectTable.id}/views`)
      .send({
        name: `${prefix} 系统筛选`,
        type: 'GRID',
        config: {
          filters: [{ fieldKey: 'code', operator: 'EQ', value: `${prefix}-VIEW-SYSTEM` }],
        },
      })
      .expect(201);
    await authenticated.agent
  ...
  it('lists, inspects and instantiates the five business templates without seed records', async () => {
    const templates = await authenticated.agent.get('/api/base/templates').expect(200);
  ...
  it('previews and commits CSV imports, downloads errors, and exports CSV/XLSX', async () => {
    ...
    const amountField = await authenticated.agent
      .post(`/api/base/tables/${tableId}/fields`)
      .send({ key: 'amount', name: '数量', type: 'NUMBER', sequence: 1 })
      .expect(201);

    const upload = await authenticated.agent
    ...
    const commit = await authenticated.agent
    ...
    await authenticated.agent.delete(`/api/base/imports/${importId}`).expect(204);
  });
});
