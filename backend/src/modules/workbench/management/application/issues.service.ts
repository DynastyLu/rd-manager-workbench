...
export class IssuesService {
  ...
  async list(query: ListIssuesQueryDto) { const page = query.page ?? 1; const pageSize = Math.min(query.pageSize ?? 20, 100); const principal = this.requestContext.requirePrincipal(); const where: Prisma.IssueWhereInput = { AND: [{ archivedAt: null, ...(query.projectId ? { projectId: query.projectId } : {}), ...(query.status ? { status: query.status } : {}), ...(query.overdue ? { dueAt: { lt: new Date() }, status: { notIn: [IssueStatus.CLOSED, IssueStatus.RESOLVED] } } : {}) }, this.dataScope.issues(principal, 'issue.read')] }; const [data, total] = await this.prisma.$transaction([this.prisma.issue.findMany({ where, orderBy: [{ updatedAt: 'desc' }, { id: 'desc' }], skip: (page-1)*pageSize, take: pageSize }), this.prisma.issue.count({ where })]); return { data, meta: { page, pageSize, total } }; }
  async get(id: string) { const principal = this.requestContext.requirePrincipal(); const entity = await this.prisma.issue.findFirst({ where: { AND: [{ id, archivedAt: null }, this.dataScope.issues(principal, 'issue.read')] } }); if (!entity) throw this.notFound(); return entity; }
