...
export class RisksService {
  constructor(
    ...
    private readonly health: ProjectHealthSnapshotService,
  ...
  async list(query: ListRisksQueryDto) {
    ...
    const where: Prisma.RiskWhereInput = { AND: [{ archivedAt: null, ...(query.projectId ? { projectId: query.projectId } : {}), ...(query.status ? { status: query.status } : {}), ...(query.level ? { level: query.level } : {}) }, this.dataScope.risks(principal, 'risk.read')] };
  ...
  async get(id: string) { const principal = this.requestContext.requirePrincipal(); const entity = await this.prisma.risk.findFirst({ where: { AND: [{ id, archivedAt: null }, this.dataScope.risks(principal, 'risk.read')] } }); if (!entity) throw this.notFound(); return entity; }
  ...
  async createRiskInTransaction(tx: Prisma.TransactionClient, dto: CreateRiskDto) {
    ...
    const entity = await tx.risk.create({ data: { ...this.fields(dto), status: dto.status ?? RiskStatus.OPEN, closedAt: dto.status === RiskStatus.CLOSED ? new Date() : null, createdByUserId: principal.userId, updatedByUserId: principal.userId, ownerUserId: principal.userId } as Prisma.RiskUncheckedCreateInput });
    if (entity.projectId) await this.health.recalculate(tx, entity.projectId); return entity;
  }
  async update(id: string, dto: UpdateRiskDto) { const principal = this.requestContext.requirePrincipal(); return this.prisma.$transaction(async (tx) => { const old = await tx.risk.findFirst({ where: { AND: [{ id, archivedAt: null }, this.dataScope.risks(principal, 'risk.update')] } }); if (!old) throw this.notFound(); const refs = { projectId: dto.projectId ?? old.projectId ?? undefined, milestoneId: dto.milestoneId ?? old.milestoneId ?? undefined, taskId: dto.taskId ?? old.taskId ?? undefined }; await this.references.assertReference(tx, refs); const status = dto.status ?? old.status; const entity = await tx.risk.update({ where: { id }, data: { ...this.fields(dto), ...refs, status, closedAt: status === RiskStatus.CLOSED ? (old.closedAt ?? new Date()) : null, updatedByUserId: principal.userId } }); for (const projectId of new Set([old.projectId, entity.projectId].filter(Boolean) as string[])) await this.health.recalculate(tx, projectId); return entity; }); }
  async archive(id: string) { const principal = this.requestContext.requirePrincipal(); await this.prisma.$transaction(async (tx) => { const entity = await tx.risk.findFirst({ where: { AND: [{ id, archivedAt: null }, this.dataScope.risks(principal, 'risk.delete')] } }); if (!entity) throw this.notFound(); await tx.risk.update({ where: { id }, data: { archivedAt: new Date() } }); if (entity.projectId) await this.health.recalculate(tx, entity.projectId); }); }
