...
export class RisksService {
  ...
  async list(query: ListRisksQueryDto) {
    ...
    const where: Prisma.RiskWhereInput = { AND: [{ archivedAt: null, ...(query.projectId ? { projectId: query.projectId } : {}), ...(query.status ? { status: query.status } : {}), ...(query.level ? { level: query.level } : {}) }, this.dataScope.risks(principal)] };
  ...
  async get(id: string) { const principal = this.requestContext.requirePrincipal(); const entity = await this.prisma.risk.findFirst({ where: { AND: [{ id, archivedAt: null }, this.dataScope.risks(principal)] } }); if (!entity) throw this.notFound(); return entity; }
